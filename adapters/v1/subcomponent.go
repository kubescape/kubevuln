...
// policySubcomponents returns the PURL scope recorded on a policy, or nil when the policy
// applies at product scope.
//
...
// from either source is read the same way.
func policySubcomponents(p armotypes.VulnerabilityExceptionPolicy) []string {
...
func scopedToSubcomponent(policies []armotypes.VulnerabilityExceptionPolicy, purl string) []armotypes.VulnerabilityExceptionPolicy {
	...
	for _, p := range policies {
		subcomponents := policySubcomponents(p)
...
func purlMatches(subcomponent, purl string) bool {
	...
	return sub.Version == "" || sub.Version == pkg.Version
}
