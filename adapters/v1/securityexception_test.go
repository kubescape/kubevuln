...
func TestConvertVulnerabilityExceptions_ExpiredUnmatchedTarget(t *testing.T) {
	...
	clusterExceptions := []sev1beta1.ClusterSecurityException{
		{
			ObjectMeta: metav1.ObjectMeta{Name: "expired-unmatched-cluster"},
			Spec: sev1beta1.SecurityExceptionSpec{
				Match: sev1beta1.ExceptionMatch{
					Resources: []sev1beta1.ResourceMatch{{Kind: "Deployment", Name: "other"}},
				},
	...
	policies, stats := ConvertToVulnerabilityExceptionPolicies(exceptions, clusterExceptions, ExceptionTarget{Kind: "Deployment", Name: "target-workload"})
	...
	assert.Equal(t, 1, stats.ExpiredBySource["ClusterSecurityException"], "cluster expired count must be recorded even when target match fails")
}
