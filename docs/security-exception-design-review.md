...
> **Resolution**: Events are covered — scanners emit Kubernetes Events on SecurityException resources when exceptions match during scans (see point 6). Prometheus metrics are covered — `kubevuln_exceptions_matched_total`, `kubevuln_exceptions_expired_total` (both labeled by `sourceKind`), and `kubevuln_exceptions_active` are exposed on `/metrics` (see [API.md](API.md#metrics)). Dry-run preview is deferred as a future enhancement. Users can validate CRD schema via `kubectl apply --dry-run=server` and observe matching behavior through Events, and now `/metrics`, after the next scan.
