...
| `kubevuln_scan_fallbacks_total` | counter | `component` (`in_process`/`sidecar`), `category` (`registry_auth`/`platform`/`size_classification`/`pull_semaphore`), `strategy` (`anonymous`/`ecr`/`gcp_adc`/`image_too_large`/`incomplete`/`platform_mismatch`/`sbom_too_large`), `outcome` (`classified`/`failed`/`succeeded`) | Fallbacks taken while resolving or classifying a scan, such as retrying a 401 with cloud credentials or falling back to anonymous access. `category="pull_semaphore"` (in-process adapter only) means the scan gave up waiting to acquire the pull/cataloging semaphore because a *previous* scan is still holding it — distinct from an ordinary slow pull or cataloging pass (`category="size_classification"`), since it signals that some other, unrelated scan is stuck |
